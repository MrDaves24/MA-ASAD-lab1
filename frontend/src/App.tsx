...
function Droppable(props: {id: string, className?: string, children?: string, data: [number, number], style?: {backgroundColor: string}}) {
    ...
    return (
        <td style={props.style} className={props.className} ref={setNodeRef}>
            {props.children}
        </td>
    );
}
                ...
                {range(config.height).map((y: number) => (<tr key={`line-${y}`}>
                    {range(config.width).map((x: number) => {
                        if (config.start[0] === x && config.start[1] === y) {
                            ...
                            return (<Droppable style={style} data={[x, y]} id={`cell-${key}`} key={`cell-${key}`}
                                               className={className.join(" ")}>{content}</Droppable>)
                        }
                    })}
                </tr>))}
                </tbody>
